...
# Task Definitions (example for 5 tasks for Scenario1) 
tasks = [
    {"label": "subtask 1", "output_words": random.randint(1000, 5000), "trans_time": random.randint(2, 10), "deadline": random.randint(50, 200)},
    {"label": "subtask 2", "output_words": random.randint(500, 3000), "trans_time": random.randint(2, 8), "deadline": random.randint(30, 150)},
    {"label": "subtask 3", "output_words": random.randint(2000, 7000), "trans_time": random.randint(3, 12), "deadline": random.randint(60, 250)},
    {"label": "subtask 4", "output_words": random.randint(1500, 6000), "trans_time": random.randint(4, 15), "deadline": random.randint(40, 220)},
    {"label": "subtask 5", "output_words": random.randint(800, 4000), "trans_time": random.randint(2, 9), "deadline": random.randint(25, 120)},
]

# Example Constants for BLOOM-176B
...
# Calculate total task size (S_{j,m}) in bytes
S_jm = sum(task['output_words'] * BYTES_PER_WORD for task in tasks)

# Assign budget and deadline
P_jm = 10000  # Assumed higher budget for flexibility
T_jm = 90     # Maximum deadline for the tasks
...
# Function to select the winner in a single round and print scores and rewards
def auction_round(bids, P_jm, T_jm, total_layers, remaining_layers):
    layers = [bid['l_ijm'] for bid in bids]
    computation_times = [bid['T_comp_ijm'] for bid in bids]
    prices = [bid['P_ijm'] for bid in bids]

    layers_ratios = [total_layers / l if l != 0 else float('inf') for l in layers]
    time_ratios = [t / l if l != 0 else float('inf') for t, l in zip(computation_times, layers)]
    price_ratios = [p / l if l != 0 else float('inf') for p, l in zip(prices, layers)]

    norm_layers_ratios = normalize(layers_ratios)
    norm_time_ratios = normalize(time_ratios)
    norm_price_ratios = normalize(price_ratios)

    valid_bids = []
    alpha, beta, gamma = 0.1, 0.3, 0.6  # The defined alpha, beta, gamma parameters in Algorithm1

    print("\nGPU Scores and Rewards in This Round:")

    for i, bid in enumerate(bids):
        if bid['P_ijm'] <= P_jm and bid['T_comp_ijm'] <= T_jm:
            score = (
                alpha * norm_layers_ratios[i] + 
                beta * norm_time_ratios[i] + 
                gamma * norm_price_ratios[i]
            )
            bid['score'] = score
            valid_bids.append(bid)
            print(f"GPU: {bid['gpu_id']}, Score: {bid['score']:.4f}, Reward: {bid['P_ijm']}\n\t(L_m / l_ijm)_norm: {norm_layers_ratios[i]:.4f}, (T_comp / l_ijm)_norm: {norm_time_ratios[i]:.4f}, (P_ijm / l_ijm)_norm: {norm_price_ratios[i]:.4f}")

    valid_bids.sort(key=lambda x: x['score'])

    if len(valid_bids) > 1:
        first_winner = valid_bids[0]
        second_winner = valid_bids[1]
        if gamma > alpha + beta:
            if remaining_layers < first_winner['l_ijm']:
                price_second_winner = second_winner['P_ijm']
                layers_second_winner = second_winner['l_ijm']
                layers_first_winner = remaining_layers
                first_winner_reward = (price_second_winner / layers_second_winner) * layers_first_winner
                first_winner['reward'] = first_winner_reward
            else:
                first_winner['reward'] = first_winner['P_ijm']
        else:
            if first_winner['P_ijm'] / first_winner['l_ijm'] == min(p / l for p, l in zip(prices, layers) if l > 0):
                price_second_winner = second_winner['P_ijm']
                layers_second_winner = second_winner['l_ijm']
                layers_first_winner = remaining_layers
                first_winner_reward = (price_second_winner / layers_second_winner) * layers_first_winner
                ...
                avg_price_per_layer = sum(p / l for p, l in zip(prices, layers) if l > 0) / len(prices)
                delta = 0.1  # Penalty parameter
                adjusted_reward = max(
                    (second_winner['P_ijm'] * first_winner['l_ijm'] / second_winner['l_ijm']),
                    first_winner['P_ijm']
                ) - delta * first_winner['l_ijm'] * (
                    (first_winner['P_ijm'] / first_winner['l_ijm']) - avg_price_per_layer
                )
                first_winner['reward'] = adjusted_reward
        print(f"First Winner: GPU {first_winner['gpu_id']}, Reward: {first_winner['reward']:.2f}")
        print(f"Second Winner: GPU {second_winner['gpu_id']}, Proposed Price: {second_winner['P_ijm']:.2f}")
        return first_winner, second_winner
    elif len(valid_bids) == 1:
        first_winner = valid_bids[0]
        first_winner['reward'] = first_winner['P_ijm']
        print(f"First Winner: GPU {first_winner['gpu_id']}, Reward: {first_winner['reward']:.2f}")
        return first_winner, None
    else:
        return None, None

# Function to simulate the multi-round auction
def multi_round_auction(bids, P_jm, T_jm, total_layers):
    selected_gpus = []
    ...
    while remaining_layers > 0:
        print(f"\nRound {round_count} auction")

        # Run auction round
        first_winner, second_winner = auction_round(bids, P_jm, T_jm, total_layers, remaining_layers)
        if first_winner:
            allocated_layers = min(first_winner['l_ijm'], remaining_layers)
            total_time_spent = allocated_layers * (first_winner['T_comp_ijm'] / first_winner['l_ijm'])

            # Add communication time to total
            if selected_gpus:
                prev_gpu = selected_gpus[-1]['gpu_id']
                current_gpu = first_winner['gpu_id']
                total_communication_time += communication_matrix[prev_gpu][current_gpu]

            selected_gpus.append({
                "gpu_id": first_winner['gpu_id'],
                "type": first_winner['type'],
                "allocated_layers": allocated_layers,
                "reward": first_winner['reward'],
                "comp_time": first_winner['T_comp_ijm'],
                "total_time_spent": total_time_spent
            })
            total_cost += first_winner['reward']
            remaining_layers -= allocated_layers

            # Remove the winner from the next round
            bids = [bid for bid in bids if bid['gpu_id'] != first_winner['gpu_id']]
        else:
            print("No valid bids found for this round!")
            break

        round_count += 1

    # Add final communication time to client
    if selected_gpus:
        last_gpu = selected_gpus[-1]['gpu_id']
        total_communication_time += client_times[last_gpu]
    ...
    total_processing_time = sum(gpu['total_time_spent'] for gpu in selected_gpus)
    communication_overhead = total_communication_time / (total_communication_time + total_processing_time)

    print("\nFairness Index (Layers) for bloom:", fairness_index_layers)
    print("Fairness Index (Rewards) for bloom:", fairness_index_rewards)
    print("Cost Efficiency for bloom:", cost_efficiency)
    print("Total Estimated Communication Time for bloom:", total_communication_time)
    print("Total Estimated Processing Time for bloom:", total_processing_time)


    return selected_gpus, total_cost

# Generate bids for the GPUs with normalized computation time
bids = generate_bids()

# Run the multi-round auction
selected_gpus, total_cost = multi_round_auction(bids, P_jm, T_jm, BLOOM_NUM_LAYERS)

# Print all bids
print("\nAll GPU Bids:")
for bid in fixed_bids:
    print(f"GPU: {bid['gpu_id']}, Type: {bid['type']}, Price: {bid['P_ijm']}, Layers: {bid['l_ijm']}, Comp Time: {bid['T_comp_ijm']}\n")

# Print selected GPUs
print("\nSelected GPUs for the task for bloom:")
for gpu in selected_gpus:
    print(f"GPU: {gpu['gpu_id']}, Type: {gpu['type']}, Allocated Layersfor bloom: {gpu['allocated_layers']}, Cost for bloom: {gpu['reward']:.2f}, Total Time Spent: {gpu['total_time_spent']:.2f}")
